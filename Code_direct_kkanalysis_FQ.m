...
for i=1:d
    for j=1:d
        if l(i,1)==l0 + ds*j
            ...
            dn(i,j) = (kl(j,1)/(l0+j*ds))*(l(i,1)^2/(l(i,1)^2-(l0+j*ds)^2));%----------------------------------------------------Declaration of Integrand Matrix
        end
    end
end
...
for i=1:d
n(i,1)=1+(2/pi)*((l(d,1)-l(d-1,1))*(s(i,1)-dn(i,1)/2-dn(i,d)/2));%------------------------------------------------------------Integrating with Trapezoidal rule
end
